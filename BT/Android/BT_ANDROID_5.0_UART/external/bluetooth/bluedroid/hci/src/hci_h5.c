...
Notes:
       This is designed for UART H5 HCI Interface in Android 4.4/5.0
*****************************************************************************/
...
#define LogMsgRtk(param, ...) { if (h5_log_enable == 1) ALOGD(param, ## __VA_ARGS__); }
...
static void h5_complete_rx_pkt(tHCI_H5_CB *h5)
{
    ...
    LogMsgRtk("SeqNumber(%d), AckNumber(%d)", H5_HDR_SEQ(h5_hdr), H5_HDR_ACK(h5_hdr));
