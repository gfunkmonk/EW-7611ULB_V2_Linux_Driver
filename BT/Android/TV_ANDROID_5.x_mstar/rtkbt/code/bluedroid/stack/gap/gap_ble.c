...
#ifdef BLUETOOTH_RTK
    if(op == GATT_UUID_GAP_PREF_CONN_PARAM) {
        memset(&p_clcb->gap_hold_link,0, sizeof(TIMER_LIST_ENT));
        p_clcb->gap_hold_link.param = (TIMER_PARAM_TYPE) p_clcb;
        if(!bdcmp(btm_cb.pairing_bda, p_clcb->bda)) {
            GATT_RemoveLinkFlag (p_clcb->conn_id);
        }
        else {
            btu_start_timer(&p_clcb->gap_hold_link,BTU_TTYPE_GAP_HOLD_LINK,1);
            GAP_TRACE_EVENT("gap_ble_cl_op_cmpl, start timer gap hold link");
        }
    }
#endif
...
#ifdef BLUETOOTH_RTK
static void GAP_BleReleaseLink (tGAP_CLCB *p_clcb)
{
    GAP_TRACE_API("GAP_BleReleaseLink");
    if(!bdcmp(btm_cb.pairing_bda, p_clcb->bda)) {
        GATT_RemoveLinkFlag (p_clcb->conn_id);
    }
    else{
        GAP_TRACE_API("GAP disconnect");
        GATT_Disconnect(p_clcb->conn_id);
    }
}


void GAP_ProcessTimeout(TIMER_LIST_ENT  *p_tle)
{
    GAP_TRACE_API("GAP_ProcessTimeout");
    switch (p_tle->event)
    {
        case BTU_TTYPE_GAP_HOLD_LINK:
        {
            GAP_BleReleaseLink ((tGAP_CLCB *)p_tle->param);
        }
        break;

        default:
            GAP_TRACE_API("Unknow time out event");
