                               ...
                               void *p_ref_data)
{
   ...
   tACL_CONN         *p = btm_bda_to_acl(bd_addr, transport);
    ...
    BTM_TRACE_API ("Security Manager: BTM_SetEncryption Handle:%d State:%d Flags:0x%x Required:0x%x",
                    transport == BT_TRANSPORT_LE ? p_dev_rec->ble_hci_handle : p_dev_rec->hci_handle, p_dev_rec->sec_state, p_dev_rec->sec_flags,
                    p_dev_rec->security_required);
...
void btm_sec_disconnected (UINT16 handle, UINT8 reason)
{
         ...
         && (memcmp (btm_cb.pairing_bda, p_dev_rec->bd_addr, BD_ADDR_LEN) == 0))
    {
        ...
        if (btm_cb.api.p_auth_complete_callback)
        {
            ...
            {
                /*
                            default result - HCI_ERR_AUTH_FAILURE will cause HID/Audio device
                            to be added into dynamic black list,  set result = reason;
                            or we can remove dynamic black list.
                            */
                BTM_TRACE_EVENT("btm_sec_disconnected: set result to reason");
                result = reason;
            }
#endif
            (*btm_cb.api.p_auth_complete_callback) (p_dev_rec->bd_addr,     p_dev_rec->dev_class,
