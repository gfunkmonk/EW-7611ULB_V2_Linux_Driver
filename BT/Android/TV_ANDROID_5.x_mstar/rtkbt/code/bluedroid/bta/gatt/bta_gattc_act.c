...
void bta_gattc_process_api_open_cancel (tBTA_GATTC_CB *p_cb, tBTA_GATTC_DATA * p_msg)
{
    ...
    APPL_TRACE_DEBUG("bta_gattc_process_api_open_cancel is_direct : %d", p_msg->api_cancel_conn.is_direct);
...
void bta_gattc_cancel_bk_conn(tBTA_GATTC_API_CANCEL_OPEN *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_gattc_cancel_bk_conn: RemBdAddr: %02x%02x%02x%02x%02x%02x",
                    p_data->remote_bda[0], p_data->remote_bda[1], p_data->remote_bda[2],
                    p_data->remote_bda[3], p_data->remote_bda[4], p_data->remote_bda[5]);

    APPL_TRACE_DEBUG("bta_gattc_cancel_bk_conn client_if is %d", p_data->client_if)
...
#ifdef BLUETOOTH_RTK
    APPL_TRACE_DEBUG("bta_gattc_disc_close close the connection p_clcb->state = %d", p_clcb->state);
