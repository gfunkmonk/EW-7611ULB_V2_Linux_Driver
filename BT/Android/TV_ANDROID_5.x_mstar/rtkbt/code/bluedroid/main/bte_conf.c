...
#ifdef BLUETOOTH_RTK
  rtkbt_logfilter = config_get_int(config, CONFIG_DEFAULT_SECTION, "RtkbtLogFilter", 1);
  h5_log_enable = config_get_int(config, CONFIG_DEFAULT_SECTION, "H5LogOutput", 0);
#ifdef BLUETOOTH_RTK_COEX
  coex_log_enable = config_get_int(config, CONFIG_DEFAULT_SECTION, "BtCoexLogOutput", 0);
#endif
#ifdef BLUETOOTH_RTK_BQB
  cert_conf(config_get_string(config, CONFIG_DEFAULT_SECTION, "CertConf", "0"));
#endif
#endif
