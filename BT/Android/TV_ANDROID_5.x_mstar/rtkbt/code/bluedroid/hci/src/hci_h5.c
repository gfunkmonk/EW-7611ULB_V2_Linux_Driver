...
#define RTKBT_RELEASE_NAME "20210702_TV_ANDROID_5.x_mstar"
#include <utils/Log.h>
#include <stdlib.h>
#include <fcntl.h>
...
extern bool hci_logging_enabled;
...
typedef struct _H5_PKT_HEADER
{
    ...
    uint8_t HdrChecksum;
}__attribute__ ((packed)) H5_PKT_HEADER;
...
}__attribute__ ((packed)) H5_CONFIG_FIELD;
...
{
    LogMsgRtk("BT_ADDR: 0x%02X%02X%02X%02X%02X%02X",ba.b[5], ba.b[4], ba.b[3], ba.b[2], ba.b[1], ba.b[0]);
}
// reverse bit
...
static sk_buff * h5_prepare_pkt(struct tHCI_H5_CB *h5, uint8_t *data, signed long len, signed long pkt_type)
{
    ...
    LogMsgRtk("We request packet no(%u) to card", h5->rxseq_txack);
    LogMsgRtk("Sending packet with seqno %u and wait %u", h5->msgq_txseq, h5->rxseq_txack);
    if (rel)
    {
        // set reliable pkt bit and SeqNumber
        hdr[0] |= 0x80 + h5->msgq_txseq;
        //LogMsgRtk("Sending packet with seqno(%u)", h5->msgq_txseq);
        ++(h5->msgq_txseq);
...
static void h5_remove_acked_pkt(struct tHCI_H5_CB *h5)
{
    ...
    if (h5->rxack != seqno)
    {
        LogMsgRtk("Peer acked invalid packet");
    }
    ...
    if (i != pkts_to_be_removed)
    {
        LogMsgRtk("Removed only (%u) out of (%u) pkts", i, pkts_to_be_removed);
    }
...
static void hci_h5_send_pure_ack(void)
{
...
#if H5_TRACE_DATA_ENABLE
    {
        ...
        LogMsgRtk("H5 TX: length(%d)", skb_get_data_length(nskb));
    ...
    )
{
    ...
    if ((skb = (sk_buff*)skb_dequeue_head(rtk_h5.unrel)) != NULL)
    {
        ...
        if (nskb)
        {
            LogMsgRtk("h5_dequeue12");
            skb_free(&skb);
            return nskb;
        }
        else
        {
            LogMsgRtk("h5_dequeue13");
...
//    LogMsgRtk("RtbGetQueueLen(rtk_h5.unack) = (%d), sliding_window_size = (%d)", RtbGetQueueLen(rtk_h5.unack), rtk_h5.sliding_window_size);

    if (RtbGetQueueLen(rtk_h5.unack)< rtk_h5.sliding_window_size &&
        (skb = (sk_buff *)skb_dequeue_head(rtk_h5.rel)) != NULL)
    {
        LogMsgRtk("h5_dequeue21");
        sk_buff *nskb = h5_prepare_pkt(&rtk_h5,
                                         skb_get_data(skb),
                                         skb_get_data_length(skb),
                                         skb_get_pkt_type(skb));
        if (nskb)
        {
            LogMsgRtk("h5_dequeue22");
            skb_queue_tail(rtk_h5.unack, skb);
            h5_start_data_retrans_timer();
            LogMsgRtk("h5_dequeue23");
            return nskb;
        }
        else
        {
            LogMsgRtk("h5_dequeue24");
...
static void h5_wake_up()
{
    ...
    while (NULL != (skb = h5_dequeue()))
    {
...
#if H5_TRACE_DATA_ENABLE
        {
            uint32_t iTemp = 0;
            uint32_t iTempTotal = 16;
            LogMsgRtk("H5 TX: length(%d)", data_len);
...
static uint8_t acl_rx_frame_end_chk (sk_buff * skb_acl)
{
    ...
    STREAM_TO_UINT16 (hci_len, data);
    STREAM_TO_UINT16 (l2cap_len, data);
    LogMsgRtk("handle(0x%x), hci_len(%d), l2cap_len(%d)", handle, hci_len, l2cap_len);
    ...
    if (hci_len > 0)
    {
        if (l2cap_len > (data_len - (HCI_ACL_PREAMBLE_SIZE+L2CAP_HEADER_SIZE)) )
        {
            ...
            frame_end = FALSE;
            LogMsgRtk("H5: L2CAP fragment PKT, data_len(%d)", data_len);
        }
        else
        {
            LogMsgRtk("H5: L2CAP Complete PKT, data_len(%d)", data_len);
        }
    }
...
void h5_process_ctl_pkts(void)
{
    ...
    if(rtk_h5.link_estab_state == H5_UNINITIALIZED) {  //sync
        ...
        if (!memcmp(skb_get_data(skb), h5sync, 0x2)) {
            ...
            LogMsgRtk("rtk_h5.sliding_window_size(%d), oof_flow_control(%d), dic_type(%d)",
         ...
         if(rtk_h5.dic_type)
            ...
            LogMsgRtk("H5_INITIALIZED receive event, ingnore");
        }
    }
    else if(rtk_h5.link_estab_state == H5_ACTIVE) {
        if (!memcmp(skb_get_data(skb), h5sync, 0x2)) {

            LogMsgRtk("H5: <<<---recv sync req in H5_ACTIVE");
            kill(getpid(), SIGKILL);
            hci_h5_send_sync_resp();
            LogMsgRtk("H5 : H5_ACTIVE transit to H5_UNINITIALIZED");
            rtk_h5.link_estab_state = H5_UNINITIALIZED;
            hci_h5_send_sync_req();
            h5_start_sync_retrans_timer();
        }
        else if (!memcmp(skb_get_data(skb), h5conf, 0x2)) {
             LogMsgRtk("H5: <<<---recv conf req in H5_ACTIVE");
             hci_h5_send_conf_resp();
        }
        else if (!memcmp(skb_get_data(skb), h5confresp,  0x2)) {
            LogMsgRtk("H5: <<<---recv conf resp in H5_ACTIVE, discard");
        }
        else {
            LogMsgRtk("H5_ACTIVE receive unknown link control msg, ingnore");
        }
...
uint8_t internal_event_intercept_h5(void)
{
    ...
    event_code = *p++;
    len = *p++;
    LogMsgRtk("event_code(0x%x)", event_code);
    ...
    if (opcode == HCI_CMD_VNDR_HEARTBEAT)
    {
        ...
        if (status == 0)
        {
            ...
            ALOGD("Heartbeat event status: %d Seq %04x", status, seqno);
    ...
    if (opcode == HCI_READ_BUFFER_SIZE)
    {
        ...
        if (status == 0)
        {
            ...
            LogMsgRtk("hc_acl_data_size:(%d), hc_sco_data_size:(%d), hc_acl_total_num:(%d), hc_sco_total_num(%d)", \
    ...
    if (opcode == HCI_LE_READ_BUFFER_SIZE)
    {
        ...
        if (status == 0)
        {
            ...
            LogMsgRtk("hc_ble_acl_data_size:(%d), hc_ble_acl_total_num:(%d)",  \
        ...
        if (p_cb->int_cmd_rsp_pending > 0)
        {
            ...
            LogMsgRtk("CommandCompleteEvent for command (0x%04X)", opcode);
    ...
    else if(event_code == HCI_NUM_OF_CMP_PKTS_EVT)
    {
        ...
        for (i = 0; i< num_of_handle; i++)
        {
            ...
            if (phci_conn)
            {
                ...
                LogMsgRtk("NumOcp(%d), hc_cur_acl_total_num(%d)", NumOcp, rtk_h5.hc_cur_acl_total_num);
    ...
    else if(event_code == HCI_CONNECTION_COMP_EVT)
    {
        ...
        LogMsgRtk("CCP EVT: status(%d), handle(0x%x), link_type(0x%x), encrypt_enabled(%d)", status, handle, link_type, encrypt_enabled);
        baPrint(bd_addr);
        ...
        if(status == 0)
        {
            ...
            LogMsgRtk("hc_cur_acl_total_num(%d)", rtk_h5.hc_cur_acl_total_num);
        }
    ...
    else if(event_code == HCI_BLE_EVT)
    {
        ...
        HCI_CONN * hci_conn = NULL;
        ble_sub_code = *p++;
        LogMsgRtk("HCI_BLE_EVT with sub event code 0x%x", ble_sub_code);
    ...
    else if(event_code == HCI_DISCONNECTION_COMP_EVT)
    {
        ...
        STREAM_TO_UINT16(handle, p);
        reason = *p;
        LogMsgRtk("DCP EVT: status(%d), handle(0x%x), reason(0x%x)", status, handle, reason);
        LogMsgRtk("hc_cur_acl_total_num(%d)", rtk_h5.hc_cur_acl_total_num);
        {
            hci_conn = ConnHashLookupByHandle(&rtk_h5, handle);
            if(hci_conn)
        ...
        event_code == 0x2f)
    {
        ...
        {
            LogMsgRtk("H5: Update CAL Inquiry success info to cal file");
...
static void hci_recv_frame(sk_buff *skb, uint8_t pkt_type)
{
    ...
    LogMsgRtk("UART H5 RX: length = %d", data_len);

#if H5_TRACE_DATA_ENABLE
    {
        uint32_t iTemp = 0;
        uint32_t iTempTotal = 16;
        LogMsgRtk("H5 RX: length(%d)", data_len);
        if(iTempTotal > data_len)
        {
            iTempTotal = data_len;
        }
    ...
    LogMsgRtk("intercepted = %d", intercepted);
...
uint8_t hci_rx_dispatch_by_handle(sk_buff* rx_skb)
{
    ...
    LogMsgRtk("hci_rx_dispatch_by_handle: (0x%x)",handle);
...
static void h5_complete_rx_pkt(struct tHCI_H5_CB *h5)
{
    ...
    //LogMsgRtk("HCI 3wire h5_complete_rx_pkt");
    h5_hdr = (H5_PKT_HEADER * )skb_get_data(h5->rx_skb);
    LogMsgRtk("SeqNumber(%d), AckNumber(%d)", h5_hdr->SeqNumber, h5_hdr->AckNumber);

    if (h5_hdr->ReliablePkt)
    {
        LogMsgRtk("Received reliable seqno %u from card", h5->rxseq_txack);
...
#ifdef BLUETOOTH_RTK_HEARTBEAT
            {
                uint8_t *p_cc = (uint8_t *)(p_cb->p_rcv_msg + 1) +
                p_cb->p_rcv_msg->offset;
...
static int h5_recv(struct tHCI_H5_CB *h5, void *data, int count)
{
    ...
    //LogMsgRtk("count %d rx_state %d rx_count %ld", count, h5->rx_state, h5->rx_count);
    ...
    while (count)
    {
        ...
        case H5_W4_DATA:
            ...
            else
            {
                ...
                //LogMsgRtk(DF_SLIP,("--------> H5_W4_DATA ACK\n"));
            }
            continue;
...
void get_acl_data_length_cback_h5(void *p_mem)
{
    ...
    LogMsgRtk("get_acl_data_length_cback_h5: opcode(0x%x)", opcode);
    if (opcode == HCI_READ_BUFFER_SIZE)
    {
        if (status == 0)
        {
...
static void data_retransfer_thread(void *arg)
{
    ...
    LogMsgRtk("data_retransfer_thread started");
    ...
    LogMsgRtk("data_retransfer_thread exiting");
    pthread_exit(NULL);
...
void hci_h5_send_msg(HC_BT_HDR *p_msg)
{
    ...
    LogMsgRtk("hci_h5_send_msg, len =%d", p_msg->len);
   ...
   if (sub_event == LOCAL_BR_EDR_CONTROLLER_ID)
    ...
    LogMsgRtk("acl_data_size = %d, acl_pkt_size = (%d), p_msg->offset=(%d)", acl_data_size, acl_pkt_size, p_msg->offset);
    ...
    if ((event == MSG_STACK_TO_HC_HCI_ACL) && (p_msg->len > acl_pkt_size))
    {
        ...
        while (p_msg->len > acl_pkt_size)
        {
            ...
            LogMsgRtk("hc_cur_acl_total_num(%d), hc_acl_total_num(%d)", rtk_h5.hc_cur_acl_total_num, rtk_h5.hc_acl_total_num);
    ...
    if (event == MSG_STACK_TO_HC_HCI_CMD)
    {
        ...
        LogMsgRtk("HCI Command opcode(0x%04X)", lay_spec);
        if(lay_spec == 0x0c03)
        {
            LogMsgRtk("RX HCI RESET Command, stop hw init timer");
                                  ...
                                  tINT_CMD_CBACK p_cback)
{
   // uint8_t * p =  (uint8_t *) (p_buf + 1);
    ...
    else if(rtk_h5.link_estab_state == H5_ACTIVE)
    {
        ...
        LogMsgRtk("hci_h5_send_int_cmd(0x%x)", opcode);
...
static void h5_timeout_handler(int signo, siginfo_t * info, void *context)
 {
    ...
    {
        LogMsgRtk("No Controller retransfer, baudrate of controller ready");
    ...
    {
        LogMsgRtk("TIMER_H5_HW_INIT_READY timeout, kill restart BT");
        kill(getpid(), SIGKILL);
...
uint16_t host_info = 0;
    ...
    LogMsgRtk("Get CAL Host info success, host_info = 0x%04x", host_info);
    ...
    LogMsgRtk("Update CAL Host info to file, host_info = 0x%04x", host_info);
