...
void poll_cleanup(void)
{
    ...
    if (bt_poll_cb.timer_created == TRUE) {
        int ret = 0;
        ret = timer_delete(bt_poll_cb.timer_id);
        if(ret != 0)
            ALOGE("poll_cleanup timer_delete fail with errno(%d)", errno);

        bt_poll_cb.timer_created = FALSE;
        ALOGI("poll_cleanup:timer_delete(bt_poll_cb.timer_id)timer_created %d", bt_poll_cb.timer_created);
    }
}
...
void poll_enable(uint8_t turn_on)
{
    ...
    if (turn_on == FALSE) {
        ALOGI("poll_enable: False  poll_cleanup()");
        poll_cleanup();
        bt_poll_cb.state = POLL_DISABLED;
    } else {
