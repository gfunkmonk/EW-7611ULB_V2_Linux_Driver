...
bool vendor_open(const uint8_t *local_bdaddr) {
  ...
  if (!vendor_interface) {
#ifdef BLUETOOTH_RTK
    ALOGE("%s unable to find symbol %s: %s", __func__, VENDOR_LIBRARY_SYMBOL_NAME, dlerror());
#else
...
#ifdef BLUETOOTH_RTK
    int status = vendor_interface->init(&vendor_callbacks, (unsigned char *)local_bdaddr,bt_hci_device_node);
#else
