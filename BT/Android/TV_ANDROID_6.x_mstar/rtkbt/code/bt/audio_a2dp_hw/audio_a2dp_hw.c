...
// MStar Android Patch Begin
#if 1
static int skt_get_available(int fd)
{
    int n = -1;
    struct pollfd pfd;
    pfd.fd = fd;
    ...
    /* poll fd to check if stack has closed the fd,
     due to an incoming suspend request.*/
    if (poll(&pfd, 1, 0) == 0)
        return 0;
...
static int skt_read(int fd, void *p, size_t len)
{
    ...
    ret = skt_get_available(fd);

     while (read < (int)len)
    {
        /* set max timeout to avoid audioflinger lockup  */
        if (poll(&pfd, 1, AUDIO_SKT_POLL_TIMEOUT_MS) == 0)
        {
            DEBUG("poll timed out (%d ms)", AUDIO_SKT_POLL_TIMEOUT_MS);
            return 0;
        }

        ret = recv(fd, (char *)p+read, (int)len-read, MSG_NOSIGNAL);
...
#if A2DPDEFAULTHZ
    sent = skt_write(out->common.audio_fd, far_buf,  bytes);
#else
    sent = skt_write(out->common.audio_fd, cap_buf,  bytes);
#endif
#else
...
// MStar Android Patch Begin
#if 1
    return AUDIO_STREAM_INPUT_BUFFER_SZ;
#else
    return 320;
#endif
// MStar Android Patch End
                       ...
                       size_t bytes)
{
    ...
    INFO("read %d bytes out of %zu bytes", read, bytes);
    return read;
}
