...
#ifdef BLUETOOTH_RTK_A2DP_DYNAMIC_TIMER
#define MTU_3DH5       (1021-8-12-1)  //hci header,l2cap header, media header,media pkt header
...
#if 1
//test <=681 HCI data size, so f/w can usb 2M packet to send.
#define MAX_PCM_SBC_PACKET_LENGTH (681-BTIF_MEDIA_AA_SBC_OFFSET)
#endif
...
void rtk_update_mtu_timer(void)
{
     ...
     APPL_TRACE_DEBUG("realtek, edr is 0x%x", edr);
     APPL_TRACE_DEBUG("realtek, btif_media_cb.TxAaMtuSize is %d", btif_media_cb.TxAaMtuSize);
     ...
     APPL_TRACE_DEBUG("realtek, output_frame_size is %d", output_frame_size);

     num_frame = btif_media_cb.TxAaMtuSize/output_frame_size;
     APPL_TRACE_DEBUG("realtek, num_frame is %d", num_frame);
     ...
     APPL_TRACE_DEBUG("realtek, pcm_bytes_per_frame is %d", pcm_bytes_per_frame);
     ...
     APPL_TRACE_DEBUG("realtek, rtk_tick is %d", rtk_tick);
}
#endif
...
#ifdef BLUETOOTH_RTK_A2DP_DYNAMIC_TIMER
    APPL_TRACE_EVENT("starting timer %dms", rtk_tick);
#else
    APPL_TRACE_EVENT("starting timer %dms", BTIF_MEDIA_TIME_TICK);
#endif

    //avoid bluetooth die when media hubup.
#ifdef BLUETOOTH_RTK
...
#ifdef BLUETOOTH_RTK_A2DP_DYNAMIC_TIMER
    alarm_set_periodic(btif_media_cb.media_alarm, rtk_tick, btif_media_task_alarm_cb, NULL);
#else
...
#ifdef BLUETOOTH_RTK_A2DP_DYNAMIC_TIMER
            UINT32 us_this_tick = rtk_tick * 1000;
#else
...
// MStar Android Patch Begin
#if 1
        while (((p_buf->len + btif_media_cb.encoder.u16PacketLength) < MAX_PCM_SBC_PACKET_LENGTH)
               && (p_buf->layer_specific < 0x0F)
               && nb_frame);
#else
...
#if 1
#if 0
        //RUN: free package if extended queue size
        if (GKI_queue_length(&btif_media_cb.TxAaQ) >= MAX_OUTPUT_A2DP_FRAME_QUEUE_SZ)
        {
            ...
            /* no more pcm to read */
            nb_frame = 0;
            APPL_TRACE_DEBUG("**RUN** no more pcm to read ");
            GKI_freebuf(GKI_dequeue(&(btif_media_cb.TxAaQ)));
            APPL_TRACE_DEBUG("**RUN** free extended buf");
        }
        else
#endif
        //RUN:to get the better performance, encode the most frame to one package.
        {
            UINT32 pcm_bytes_per_frame = btif_media_cb.encoder.s16NumOfSubBands *
                                         btif_media_cb.encoder.s16NumOfBlocks *
                                         btif_media_cb.media_feeding.cfg.pcm.num_channel *
            ...
            if(CountFrame == FALSE)
            {
                //cal max frame in one packet
                frame_num=MAX_PCM_SBC_PACKET_LENGTH/btif_media_cb.encoder.u16PacketLength;
                if(frame_num > nb_frame)
                {
                    CountFrame = TRUE;
                    btif_media_cb.media_feeding_state.pcm.counter += nb_frame * pcm_bytes_per_frame;
                    APPL_TRACE_VERBOSE("**RUN** skip nb_frame=%d",nb_frame);
                    nb_frame =0;
                }
            }
        }
