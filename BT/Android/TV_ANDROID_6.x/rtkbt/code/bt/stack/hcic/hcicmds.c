...
BOOLEAN btsnd_hcic_delete_reserved_lt_addr (UINT8 lt_addr)
{
    BT_HDR *p;
    UINT8 *pp;
...
BOOLEAN btsnd_hcic_write_sync_train_param (UINT8* p_data)
{
    ...
    ARRAY_TO_STREAM  (pp, p_data, HCIC_PARAM_SIZE_WRITE_SYNC_TRAIN_PARAM);
...
BOOLEAN btsnd_hcic_write_clb_data (UINT8* p_data, UINT8 len)
{
    ...
    if ((p = HCI_GET_CMD_BUF(HCIC_PARAM_SIZE_SET_CLB_DATA_EXCPT + len)) == NULL)
        return (FALSE);

    pp = (UINT8 *)(p + 1);

    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_SET_CLB_DATA_EXCPT + len;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_WRITE_CLB_DATA);
    UINT8_TO_STREAM  (pp, (HCIC_PARAM_SIZE_SET_CLB_DATA_EXCPT + len));

    ARRAY_TO_STREAM  (pp, p_data, (HCIC_PARAM_SIZE_SET_CLB_DATA_EXCPT + len));
...
BOOLEAN btsnd_hcic_set_clb (UINT8* p_data)
{
    ...
    if ((p = HCI_GET_CMD_BUF(HCIC_PARAM_SIZE_SET_CONNLESS_SLBC)) == NULL)
        return (FALSE);

    pp = (UINT8 *)(p + 1);

    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_SET_CONNLESS_SLBC;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_SET_CLB);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_SET_CONNLESS_SLBC);

    ARRAY_TO_STREAM  (pp, p_data, HCIC_PARAM_SIZE_SET_CONNLESS_SLBC);

    btu_hcif_send_cmd (LOCAL_BR_EDR_CONTROLLER_ID,  p);
    return (TRUE);
}

BOOLEAN btsnd_hcic_start_sync_train ()
{
    BT_HDR *p;
    UINT8 *pp;
