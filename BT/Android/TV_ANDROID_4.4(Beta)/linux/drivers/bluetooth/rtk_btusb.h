...
#define CONFIG_LENGTH 1024
    ...
    int buf_size = CONFIG_LENGTH;
    int ret = 0;
    char *head;
    char *ptr;

    if((buf = kmalloc(buf_size, GFP_KERNEL)) == NULL) {
        RTKBT_ERR("Couldn't kmalloc buffer size");
        return;
    }

    memset(buf, 0, CONFIG_LENGTH);
    ...
    ret = vfs_read(fp, buf, CONFIG_LENGTH, &fp->f_pos);
    set_fs(oldfs);
    if(ret < 0) {
