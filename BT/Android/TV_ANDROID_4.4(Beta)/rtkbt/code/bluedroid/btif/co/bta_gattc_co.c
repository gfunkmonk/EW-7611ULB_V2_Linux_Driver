...
static BOOLEAN cacheOpen(BD_ADDR bda, BOOLEAN to_save)
{
    char fname[255] = {0};
...
void bta_gattc_co_cache_load(BD_ADDR server_bda, UINT16 evt, UINT16 start_index, UINT16 conn_id)
{
    ...
    BTIF_TRACE_DEBUG5("%s() - sCacheFD=%p, start_index=%d, read=%d, status=%d",
                              ...
                              tBTA_GATTC_NV_ATTR *p_attr_list, UINT16 attr_index, UINT16 conn_id)
{
    ...
    if (sCacheFD != 0)
    {
        ...
        BTIF_TRACE_DEBUG2("%s() wrote %d", __FUNCTION__, num);
    }
#endif
