...
char bt_hci_device_node[BT_HCI_DEVICE_NODE_MAX_LEN];
...
#endif
#ifdef BLUETOOTH_RTK
void init_vnd_if(unsigned char *local_bdaddr,const char *path,const char* device_node);
#else
...
#ifdef BLUETOOTH_RTK
unsigned int rtkbt_logfilter = 1;
unsigned int rtkbt_btsnoopsaver = 0;
#endif
...
static int init(const bt_hc_callbacks_t* p_cb, unsigned char *local_bdaddr)
{
    ...
    ret = property_get("bluetooth.rtkbt_logfilter", temp, "");
    ALOGD("get rtkbt_logfilter property , ret = %d", ret);
    ALOGD("temp is %s", temp);
    if(ret > 0)
    {
        rtkbt_logfilter = atoi(temp);
        ALOGD("rtkbt_logfilter after property_get is %d", rtkbt_logfilter);
    }

    ret = property_get("bluetooth.rtkbt_btsnoopsaver", temp, "");
    ALOGD("get rtkbt_btsnoopsaver property , ret = %d", ret);
    ALOGD("temp is %s", temp);
    if(ret > 0)
    {
        rtkbt_btsnoopsaver = atoi(temp);
        ALOGD("rtkbt_btsnoopsaver after property_get is %d", rtkbt_btsnoopsaver);
    }
#endif
    ALOGI("init");
...
#ifdef BLUETOOTH_RTK
    if(strcmp(bt_hci_device_node,"/dev/rtk_btusb") == 0)
    {
        
        init_vnd_if(local_bdaddr,VENDOR_LIBRARY_NAME_USB,bt_hci_device_node);
    }
    else
        init_vnd_if(local_bdaddr,VENDOR_LIBRARY_NAME_UART,bt_hci_device_node);
#else
