...
extern unsigned int rtkbt_btsnoopsaver;
#endif
...
#ifdef BLUETOOTH_RTK
static int btsnoop_save_last(char* oldpath)
{
    char last_log_path[PATH_MAX];
    struct timeval tv;
    struct tm *p;

    if(rtkbt_btsnoopsaver == 1)
    {
        gettimeofday(&tv, NULL);
        p = localtime(&tv.tv_sec);
        snprintf(last_log_path, PATH_MAX, "%s_%04d%02d%02d-%02d%02d%02d-%06ld",
                        oldpath, 1900 + p->tm_year, 1 + p->tm_mon, p->tm_mday,
                        p->tm_hour, p->tm_min, p->tm_sec, tv.tv_usec);

        if (!rename(oldpath, last_log_path) && errno != ENOENT)
                ALOGE("%s unable to rename '%s' to '%s': %s", __func__, oldpath, last_log_path, strerror(errno));
    }
...
static int btsnoop_log_open(char *btsnoop_logfile)
{
    ...
    {
#ifdef BLUETOOTH_RTK
        btsnoop_save_last(btsnoop_logfile);
#endif
...
void btsnoop_capture(HC_BT_HDR *p_buf, uint8_t is_rcvd)
{
    ...
    switch (p_buf->event & MSG_EVT_MASK)
    {
        case MSG_HC_TO_STACK_HCI_EVT:
            SNOOPDBG("TYPE : EVT");
#ifdef BLUETOOTH_RTK
        if((rtkbt_logfilter&1)) {
            if(is_rcvd && ((*(p + 3) == 0x94) && (*(p + 4) == 0xfc) && (*(p + 5) == 0x00)))
                return;
        }
#endif
            btsnoop_hci_evt(p);
