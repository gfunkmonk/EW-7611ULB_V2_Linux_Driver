...
void bta_hh_security_cmpl(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_buf)
{
    ...
    if (p_cb->status == BTA_HH_OK)
    {
        ...
        {
            APPL_TRACE_DEBUG0("bta_hh_security_cmpl no reports loaded, try to load");
...
/*BOARD_HAVE_BLUETOOTH_RTK_COEX end*/
    ...
    {
        APPL_TRACE_DEBUG0("Notification Stolen by RTKBT\n");
        return;
    }
#endif
                                      ...
                                      UINT8 num_rpt)
{
    ...
    if (num_rpt != 0)  /* no cache is found */
    {
        ...
        for (; i <num_rpt; i ++, p_rpt_cache ++)
        {
            ...
            {
                APPL_TRACE_ERROR0("bta_hh_process_cache_rpt: allocation report entry failure");
                break;
            }
            else
            {
