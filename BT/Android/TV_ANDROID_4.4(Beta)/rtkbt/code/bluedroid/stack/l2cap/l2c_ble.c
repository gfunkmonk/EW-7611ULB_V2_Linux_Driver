...
void L2CA_HandleConnUpdateEvent (UINT16 handle, UINT8 status)
{
    ...
    if (status != HCI_SUCCESS)
    {
        ...
        if(p_lcb->upd_status & L2C_BLE_CONN_UPDATE_DISABLE)
        {
            L2CAP_TRACE_WARNING0("l2cble_process_conn_update_evt: Reset after failure");
            p_lcb->upd_status &= ~L2C_BLE_NEW_CONN_PARAM;
        }
        else
        {
            L2CAP_TRACE_WARNING0("l2cble_process_conn_update_evt: Retry after failure");
            p_lcb->upd_status |= L2C_BLE_NEW_CONN_PARAM;
        }
#endif
...
BOOLEAN l2cble_init_direct_conn (tL2C_LCB *p_lcb)
{
...
#ifdef BLUETOOTH_RTK_4_2
    if(HCI_LE_EXT_CREATE_CONNECTION_SUPPORTED(btm_cb.devcb.supported_cmds)) {
         struct EXT_CONN_PHY_CFG phy_cfg[3];  // maximum three phys
         int i;
         UINT8 initiating_phys = 0x03;   //default use 1M/2M
         if(!HCI_LE_2M_PHY_SUPPORTED((UINT8 *)&btm_cb.devcb.local_le_features))
             initiating_phys &= ~0x02;

         for (i = 0; i < 2; i++) {
           phy_cfg[i].scan_int = scan_int;
           phy_cfg[i].scan_win = scan_win;
           phy_cfg[i].conn_int_min = (UINT16) ((p_dev_rec->conn_params.min_conn_int != BTM_BLE_CONN_PARAM_UNDEF) ?
                                     p_dev_rec->conn_params.min_conn_int : BTM_BLE_CONN_INT_MIN_DEF);
           phy_cfg[i].conn_int_max = (UINT16) ((p_dev_rec->conn_params.max_conn_int != BTM_BLE_CONN_PARAM_UNDEF) ?
                                     p_dev_rec->conn_params.max_conn_int : BTM_BLE_CONN_INT_MAX_DEF);
           phy_cfg[i].conn_latency = (UINT16) ((p_dev_rec->conn_params.slave_latency != BTM_BLE_CONN_PARAM_UNDEF) ?
                                     p_dev_rec->conn_params.slave_latency : 0);
           phy_cfg[i].sup_timeout =  (UINT16) ((p_dev_rec->conn_params.supervision_tout != BTM_BLE_CONN_PARAM_UNDEF) ?
                                     p_dev_rec->conn_params.supervision_tout : BTM_BLE_CONN_SUP_TOUT_DEF);
           phy_cfg[i].min_ce_len = 0;
           phy_cfg[i].max_ce_len = 0;
         }

         if(!btsnd_hcic_ble_ext_create_conn(FALSE, own_addr_type,
                            init_addr_type, init_addr, initiating_phys,
                            phy_cfg)){
            l2cu_release_lcb (p_lcb);
    ...
    else if (!btsnd_hcic_ble_create_ll_conn (scan_int,/* UINT16 scan_int      */
...
void l2cble_update_data_length(tL2C_LCB* p_lcb) {
  ...
  if (p_lcb->tx_data_len != tx_mtu && !p_lcb->data_len_updating) {
    status = BTM_SetBleDataLength(p_lcb->remote_bd_addr, tx_mtu);
    if(status == BTM_SUCCESS)
      p_lcb->data_len_updating = TRUE;
  }
}

void l2cble_check_data_length(BD_ADDR remote_bda) {
  UINT16 tx_mtu = 0;
  UINT16 i = 0;
  tL2C_LCB* p_lcb = l2cu_find_lcb_by_bd_addr(remote_bda);
  tBTM_STATUS status = BTM_SUCCESS;
...
void l2cble_set_fixed_channel_tx_data_length(BD_ADDR remote_bda,
    ...
    L2CAP_TRACE_DEBUG2("%s TX MTU = %d", __func__, tx_mtu);

    if(!HCI_LE_DATA_LEN_EXT_SUPPORTED((UINT8 *)&btm_cb.devcb.local_le_features)){
        L2CAP_TRACE_WARNING1("%s, request not supported", __func__);
        return;
    }

    /* See if we have a link control block for the connection */
    if (p_lcb == NULL || !p_lcb->is_ble_link) return;
...
void l2cble_process_data_length_change_event(UINT16 handle, UINT16 tx_data_len,
  ...
  L2CAP_TRACE_DEBUG2("%s TX data len = %d", __func__, tx_data_len);
  if (p_lcb == NULL) return;

  if (tx_data_len > 0) p_lcb->tx_data_len = tx_data_len;
  p_lcb->data_len_updating = FALSE;
