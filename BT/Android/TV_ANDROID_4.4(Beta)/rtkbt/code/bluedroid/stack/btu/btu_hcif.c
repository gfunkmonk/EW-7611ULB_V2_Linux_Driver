...
#endif
#ifdef BLUETOOTH_RTK_ENCRY
        || (opcode == HCI_READ_ENCR_KEY_SIZE)
#endif
...
#ifdef BLUETOOTH_RTK_API
            rtkbt_api_Hook(RTKBT_HOOK_RECEIVE_HCI_VENDOR_EVENT,(void *)p_msg,0);
#endif
...
static void btm_read_encryption_key_size_after_encryption_change(void *pp) {
    UINT8 status;
    UINT16 handle;
    UINT8 key_size;
    UINT8* p = (UINT8* )pp;
    ...
    if (status != HCI_SUCCESS) {
      ALOGI("%s: disconnecting, status: %d", __func__, status);
      btsnd_hcic_disconnect(handle, HCI_ERR_PEER_USER);
      return;
    }

    if (key_size < MIN_KEY_SIZE) {
      ALOGE("%s encryption key too short, disconnecting. handle:0x%x,key_size:%d ", __func__, handle, key_size);
...
static void btu_hcif_encryption_change_evt (UINT8 *p, UINT16 evt_len)
{
    ...
    if (status != HCI_SUCCESS || encr_enable == 0 ||
      ...
      btsnd_hcic_read_encr_key_size(handle, (void *)btm_read_encryption_key_size_after_encryption_change);
    }
#else
    btm_acl_encrypt_change (handle, status, encr_enable);
    btm_sec_encrypt_change (handle, status, encr_enable);
                                           ...
                                           void *p_cplt_cback, UINT8 ctr_id)
{
    switch (opcode)
    {
        ...
        case HCI_READ_ENCR_KEY_SIZE:
            (*((tBTM_CMPL_CB *)p_cplt_cback))((void*) p);
            break;
#endif
...
/* BLE Commands */
        ...
        case HCI_BLE_READ_DEFAULT_DATA_LENGTH:
            btm_ble_read_default_data_length_complete(p, evt_len);
            break;
        case HCI_BLE_READ_MAXIMUM_DATA_LENGTH:
            btm_ble_read_maximum_data_length_complete(p, evt_len);
            break;
#endif
...
#endif
#ifdef BLUETOOTH_RTK_ENCRY
                || (cc_opcode == HCI_READ_ENCR_KEY_SIZE)
#endif
               )
            {
...
#ifdef BLUETOOTH_RTK_ENCRY
static void btm_read_encryption_key_size_after_key_refresh(void *pp) {
    UINT8 status;
    UINT16 handle;
    UINT8 key_size;
    UINT8* p = (UINT8* )pp;
...
static void btu_hcif_encyption_key_refresh_cmpl_evt (UINT8 *p, UINT16 evt_len)
{
    ...
    if (status != HCI_SUCCESS || BTM_IsBleConnection(handle)) {
      ...
      btsnd_hcic_read_encr_key_size(handle, (void *)btm_read_encryption_key_size_after_key_refresh);
    }
#else
...
static void btu_ble_data_length_change_evt(UINT8* p, UINT16 evt_len) {
    ...
    if(!HCI_LE_DATA_LEN_EXT_SUPPORTED((UINT8 *)&btm_cb.devcb.local_le_features)){
        BT_TRACE_0 (TRACE_LAYER_HCI, TRACE_TYPE_WARNING, "btu_ble_data_length_change_evt request not supported");
        return;
    }
...
static void btu_ble_proc_enhanced_conn_cmpl(UINT8* p, UINT16 evt_len) {
    btm_ble_enchance_conn_complete(p, evt_len);
}
#endif
