...
BOOLEAN btsnd_hcic_ble_ext_create_conn(UINT8 init_filter_policy,
                                    ...
                                    struct EXT_CONN_PHY_CFG* phy_cfg) {
  BT_HDR* p;
  UINT8 *pp;
  UINT8 param_len;
  int i;
  int phy_cnt = 0;
  if ((p = HCI_GET_CMD_BUF(HCIC_PARAM_SIZE_BLE_CREATE_EXT_LL_CONN)) == NULL)
        return (FALSE);

  pp = (UINT8*)(p + 1);

  if(initiating_phys & 0x01)
      phy_cnt++;

  if(initiating_phys & 0x02)
      phy_cnt++;

  if(initiating_phys & 0x04)
      phy_cnt++;


  /* param_len = initial_params + size_per_channel * num_of_channels */
  param_len = 10 + (16 * phy_cnt);
...
BOOLEAN btsnd_hcic_ble_set_default_phy(UINT8 all_phys, UINT8 tx_phys, UINT8 rx_phys) {
  ...
  if ((p = HCI_GET_CMD_BUF(HCIC_PARAM_SIZE_BLE_SET_DEFAULT_PHY)) == NULL)
        return (FALSE);

  pp = (UINT8*)(p + 1);

  /* param_len = initial_params + size_per_channel * num_of_channels */
  param_len = HCIC_PARAM_SIZE_BLE_SET_DEFAULT_PHY;
...
BOOLEAN btsnd_hcic_ble_set_phy(UINT16 handle, UINT8 all_phys,
                                     UINT8 tx_phys, UINT8 rx_phys, UINT16 phy_opt) {
  BT_HDR* p;
  UINT8 *pp;
  UINT8 param_len;
  int i;

  if ((p = HCI_GET_CMD_BUF(HCIC_PARAM_SIZE_BLE_PHY)) == NULL)
        return (FALSE);

  pp = (UINT8*)(p + 1);

  /* param_len = initial_params + size_per_channel * num_of_channels */
  param_len = HCIC_PARAM_SIZE_BLE_PHY;
  ...
  UINT8_TO_STREAM(pp, tx_phys);
  UINT8_TO_STREAM(pp, rx_phys);
  UINT16_TO_STREAM(pp, phy_opt);

  btu_hcif_send_cmd(LOCAL_BR_EDR_CONTROLLER_ID, p);
  return TRUE;
}

BOOLEAN btsnd_hcic_ble_read_suggested_default_data_length(void)
{
    BT_HDR *p;
    UINT8 *pp;
    if ((p = HCI_GET_CMD_BUF(HCIC_PARAM_SIZE_READ_CMD)) == NULL)
...
BOOLEAN btsnd_hcic_ble_read_maximum_data_length(void)
{
    BT_HDR *p;
    UINT8 *pp;
    if ((p = HCI_GET_CMD_BUF(HCIC_PARAM_SIZE_READ_CMD)) == NULL)
    ...
    UINT16_TO_STREAM (pp, HCI_BLE_READ_MAXIMUM_DATA_LENGTH);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_READ_CMD);
