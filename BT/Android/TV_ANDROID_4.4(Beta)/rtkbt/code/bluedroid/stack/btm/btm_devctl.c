...
{
    BTM_TRACE_DEBUG0("btm_read_ble_suggested_default_data_length ");
    btsnd_hcic_ble_read_suggested_default_data_length();
}

UINT16 btm_ble_suggested_default_data_length (void)
{
    return btm_cb.ble_ctr_cb.suggested_default_data_length;
}
...
void btm_read_ble_local_supported_features_complete (UINT8 *p, UINT16 evt_len)
{
    ...
    if(HCI_LE_DATA_LEN_EXT_SUPPORTED((UINT8 *)&btm_cb.devcb.local_le_features)){
        btsnd_hcic_ble_read_maximum_data_length();
    }
#endif
#endif
...
#ifdef BLUETOOTH_RTK_4_2
void btm_ble_read_default_data_length_complete (UINT8 *p, UINT16 evt_len)
{
    UINT8       status;

    BTM_TRACE_DEBUG0("btm_ble_read_default_data_length_complete ");

    STREAM_TO_UINT8  (status, p);
    if (status == HCI_SUCCESS)
    {
        STREAM_TO_UINT16(btm_cb.ble_ctr_cb.suggested_default_data_length, p);
    }
    else
    {
        BTM_TRACE_WARNING1 ("btm_ble_read_default_data_length_complete status = %d", status);
    }
}

void btm_ble_read_maximum_data_length_complete(UINT8 *p, UINT16 evt_len)
{
    UINT8       status;

    BTM_TRACE_DEBUG0("btm_ble_read_maximum_data_length_complete ");

    STREAM_TO_UINT8  (status, p);
    if (status == HCI_SUCCESS)
    {
        STREAM_TO_UINT16(btm_cb.ble_ctr_cb.supported_max_tx_octets, p);
        STREAM_TO_UINT16(btm_cb.ble_ctr_cb.supported_max_tx_time, p);
        STREAM_TO_UINT16(btm_cb.ble_ctr_cb.supported_max_rx_octets, p);
        STREAM_TO_UINT16(btm_cb.ble_ctr_cb.supported_max_rx_time, p);
        btm_read_ble_suggested_default_data_length();
    }
    else
    {
        BTM_TRACE_WARNING1 ("btm_ble_read_maximum_data_length_complete status = %d", status);
    }
