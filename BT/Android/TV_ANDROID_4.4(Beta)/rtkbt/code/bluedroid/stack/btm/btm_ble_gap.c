...
BOOLEAN btm_ble_update_inq_result(tINQ_DB_ENT *p_i, UINT8 addr_type, UINT8 evt_type, UINT8 *p)
{
    ...
    {
        BTM_TRACE_DEBUG6("btm_ble_update_inq_result: ignore scan Response[%02x:%02x:%02x:%02x:%02x:%02x]", \
            p_cur->remote_bd_addr[0],p_cur->remote_bd_addr[1],p_cur->remote_bd_addr[2],
            p_cur->remote_bd_addr[3],p_cur->remote_bd_addr[4],p_cur->remote_bd_addr[5]);
        return FALSE;
    }
#endif
...
#ifdef BLUETOOTH_RTK_4_2
            l2cble_check_data_length(p_acl_cb->remote_addr);
#endif
            break;
        }
    }
