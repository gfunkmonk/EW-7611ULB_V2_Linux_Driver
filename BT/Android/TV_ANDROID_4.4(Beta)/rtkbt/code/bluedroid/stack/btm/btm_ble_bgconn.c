...
BOOLEAN btm_add_dev_to_controller (BOOLEAN to_add, BD_ADDR bd_addr, UINT8 attr)
{
    ...
    if(started) {
        if(p_dev_rec != NULL) {
            addr_type = p_dev_rec->ble.ble_addr_type;
        }
        ...
        if (p_cb->conn_state == BLE_CONN_IDLE && btm_ble_count_unconn_dev_in_whitelist() > 0)
        {
           ...
           if(HCI_LE_EXT_CREATE_CONNECTION_SUPPORTED(btm_cb.devcb.supported_cmds)) {
                ...
                for (i = 0; i < 2; i++) {
                  ...
                  phy_cfg[i].conn_int_min = BTM_BLE_CONN_INT_MIN_DEF;
                  phy_cfg[i].conn_int_max = BTM_BLE_CONN_INT_MIN_DEF;
                  phy_cfg[i].conn_latency = BTM_BLE_CONN_SLAVE_LATENCY_DEF;
                  phy_cfg[i].sup_timeout = BTM_BLE_CONN_TIMEOUT_DEF;
                  phy_cfg[i].min_ce_len = 0;
                  phy_cfg[i].max_ce_len = 0;
                }

                if(!btsnd_hcic_ble_ext_create_conn(0x01, own_addr_type,
                                   BLE_ADDR_PUBLIC, dummy_bda, initiating_phys,
                                   phy_cfg)){
