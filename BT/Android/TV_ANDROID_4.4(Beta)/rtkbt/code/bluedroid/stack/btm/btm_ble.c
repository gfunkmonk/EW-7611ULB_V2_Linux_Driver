 ...
 *
 ******************************************************************************/
tBTM_STATUS BTM_SetBleDataLength(BD_ADDR bd_addr,
                                 uint16_t tx_pdu_length) {
    tACL_CONN* p_acl = btm_bda_to_acl(bd_addr);

    if (p_acl == NULL || !p_acl->is_le_link) {
      BTM_TRACE_ERROR1("%s: Wrong mode: no LE link exist or LE not supported",
                      __func__);
      return BTM_WRONG_MODE;
    }

    BTM_TRACE_DEBUG2("%s: tx_pdu_length =%d", __func__, tx_pdu_length);

    if(!HCI_LE_DATA_LEN_EXT_SUPPORTED((UINT8 *)&btm_cb.devcb.local_le_features)){
        BTM_TRACE_ERROR1("%s failed, request not supported", __func__);
        return BTM_ILLEGAL_VALUE;
    }

    if (!HCI_LE_DATA_LEN_EXT_SUPPORTED(p_acl->peer_le_features)) {
        BTM_TRACE_ERROR1("%s failed, peer does not support request", __func__);
        return BTM_ILLEGAL_VALUE;
    }

    if(!HCI_LE_SET_DATA_LEN_SUPPORTED(btm_cb.devcb.supported_cmds)) {
        BTM_TRACE_ERROR1("%s failed, local does not support set data length cmd", __func__);
        return BTM_ILLEGAL_VALUE;
    }
...
#ifdef BLUETOOTH_RTK_4_2
/*****************************************************************************
**  Function        btm_ble_enchance_conn_complete
**
**  Description     LE exchance connection complete.
**
******************************************************************************/
void btm_ble_enchance_conn_complete(UINT8 *p, UINT16 evt_len)
{
    UINT8       role, status, bda_type;
    UINT16      handle;
    BD_ADDR     bda;
