...
#define RTKBT_RELEASE_NAME "20210702_TV_ANDROID_4.4(Beta)"
...
#define MAX_PATCH_SIZE_25K            (1024*25)   //24K
...
#define CONFIG_MAC_OFFSET_GEN_4PLUS     (0x30)      //MAC's OFFSET in config/efuse for rtkgeneration 4+ bluetooth chip
...
//sample code, add special settings
        case 0x015B:
            memcpy(val, txpower_cfg, txpower_len);
            ALOGI("Tx power cfg: %02x %02x %02x %02x", txpower_cfg[0], txpower_cfg[1], txpower_cfg[2], txpower_cfg[3]);
            res = txpower_len;
            break;

        case 0x01E6:
            val[0] = xtal_cfg;
            ALOGI("XTAL cfa: %02x", xtal_cfg);
            res = 1;
            break;
