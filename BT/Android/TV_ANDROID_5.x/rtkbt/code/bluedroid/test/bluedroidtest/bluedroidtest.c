...
static uint8_t bdt_show_device_ready = 0;
static uint8_t bdt_show_macaddr_ready = 0;
static uint8_t found_one = 0; //if find a device, back with address, next address will return directly
...
static void device_found(int num_properties, bt_property_t *properties) 
{
    ...
    printf("\n");
    bdt_log("find a bt device, address is ");
    printf("\n");
    int data_type = (int)properties->type;
    bt_bdaddr_t *bdaddr_data;
    uint8_t i; 
    if (data_type == 2) {
	bdaddr_data = (bt_bdaddr_t*)properties->val;
	found_one = 1;
    	for (i = 0; i < 6; i++) {
		 printf("%X ", bdaddr_data->address[i]); 
   	 }
    }
    printf("\n");
    //bdt_shutdown();
...
{
	int i, j;
	bt_bdaddr_t *localbdaddr;
	if(status != BT_STATUS_SUCCESS)
		return;
	
	if (bdt_show_macaddr_ready)
	{
		//printf("\n");
		bdt_log("the local bluetooth mac address is ");
		printf("\n");
		
		for(i = 0;i < num_properties;i++)
			if(properties[i].type == BT_PROPERTY_BDADDR)
			{
				localbdaddr = (bt_bdaddr_t*)properties->val;
				for (j = 0; j < 6; j++) 
				{		 
					printf("%X ", localbdaddr->address[j]); 
				}
				printf("\n");
				bdt_show_macaddr_ready = 0;
			}
...
{
	bdt_log("DiscoveryState : %s",(state == BT_DISCOVERY_STOPPED)?"BT_DISCOVERY_STOPPED":"BT_DISCOVERY_STARTED");
	if(found_one == 0 && state == BT_DISCOVERY_STOPPED)
		bdt_log("cannot find any device");
}
...
{
   // bdt_log("BT SCAN");
    bdt_show_device_ready = 1;
    if (!bt_enabled) {
		
		//bdt_enable();
		bdt_log("BT closed,please open BT firstly! you can use command : enable");
    }
	else{
...
const t_cmd console_cmd_list[] =
{
	...
	{ "scan", do_scan, ":: scan devices and back with a Bt device address", 0},
