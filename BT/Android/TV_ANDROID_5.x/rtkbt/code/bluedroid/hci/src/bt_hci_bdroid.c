...
extern char bt_hci_device_node[];
#else
...
#ifdef BLUETOOTH_RTK_HEARTBEAT
static void event_heartbeat_idle_timeout(UNUSED_ATTR void *context) {
    ALOGV("%s num_hci_cmd_pkts = %d", __func__, num_hci_cmd_pkts);
    ALOGW("heartbeat timeout, unack_num: %d", rtkbt_heartbeat_noack_num);
    rtkbt_heartbeat_noack_num ++;
    if(rtkbt_heartbeat_noack_num >= 3)
    {
        usleep(10000); /* 10 milliseconds */
        ALOGE("Heartbeat: EvtSeq %04x NoACK %d", rtkbt_heartbeat_evt_seqno, rtkbt_heartbeat_noack_num);
...
#ifdef BLUETOOTH_RTK_HEARTBEAT
void bthc_poll_idle_timeout(void) {
  BTHCDBG("%s before pthread_mutex_lock(&hc_cb.worker_thread_lock);", __func__);
  pthread_mutex_lock(&hc_cb.worker_thread_lock);
  BTHCDBG("%s  after pthread_mutex_lock(&hc_cb.worker_thread_lock);", __func__);
  if (hc_cb.worker_thread)
  {
    BTHCDBG("%s  before thread_post(hc_cb.worker_thread, event_heartbeat_idle_timeout, NULL)", __func__);
    thread_post(hc_cb.worker_thread, event_heartbeat_idle_timeout, NULL);
    BTHCDBG("%s  after thread_post(hc_cb.worker_thread, event_heartbeat_idle_timeout, NULL)", __func__);
  }
  pthread_mutex_unlock(&hc_cb.worker_thread_lock);
  BTHCDBG("%s  after pthread_mutex_unlock(&hc_cb.worker_thread_lock);", __func__);
}
#endif
/*******************************************************************************
**
...
static void cleanup(void)
{
    ...
    if (hc_cb.worker_thread)
    {
        ...
        BTHCDBG("%s  before pthread_mutex_lock(&hc_cb.worker_thread_lock) p_hci_if->cleanup();", __func__);
        pthread_mutex_lock(&hc_cb.worker_thread_lock);
        BTHCDBG("%s  before p_hci_if->cleanup();", __func__);
        p_hci_if->cleanup();
        BTHCDBG("%s  after p_hci_if->cleanup();", __func__);
        pthread_mutex_unlock(&hc_cb.worker_thread_lock);
        BTHCDBG("%s  pthread_mutex_unlock(&hc_cb.worker_thread_lock); p_hci_if->cleanup();", __func__);

        BTHCDBG("%s  before thread_free(hc_cb.worker_thread);", __func__);
        thread_free(hc_cb.worker_thread);
        BTHCDBG("%s  after thread_free(hc_cb.worker_thread);", __func__);
        BTHCDBG("%s  before pthread_mutex_lock(&hc_cb.worker_thread_lock)", __func__);
        pthread_mutex_lock(&hc_cb.worker_thread_lock);
        hc_cb.worker_thread = NULL;
        pthread_mutex_unlock(&hc_cb.worker_thread_lock);
        BTHCDBG("%s  pthread_mutex_unlock(&hc_cb.worker_thread_lock);", __func__);
