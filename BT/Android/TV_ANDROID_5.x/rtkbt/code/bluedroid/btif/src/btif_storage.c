...
#ifdef BLUETOOTH_RTK
#define BTIF_AUTO_PAIR_CONF_FILE  "/etc/bluetooth/rtkbt_auto_pair_devlist.conf"
#else
...
static bt_status_t btif_in_fetch_bonded_devices(btif_bonded_devices_t *p_bonded_devices, int add)
{
    ...
    do
    {
        ...
        BTIF_TRACE_DEBUG("%s, Remote device:%s, size:%d", __FUNCTION__,kname, kname_size);
        int type = BTIF_CFG_TYPE_BIN;
        LINK_KEY link_key;
        int size = sizeof(link_key);
...
bt_status_t btif_in_fetch_bonded_ble_device(char *remote_bd_addr,int add, btif_bonded_devices_t *p_bonded_devices)
{
...
#endif
    {
            ...
            if (is_device_added)
            {
                ...
                BTIF_TRACE_DEBUG("%s %s add class of device and bd_name", __FUNCTION__,remote_bd_addr);
