...
#define BTA_HH_LE_DISC_CHAR_NUM     9
#else
                            ...
                            tBTA_GATTC_READ *p_data)
{
    ...
    {
        if (p_data->char_id.uuid.uu.uuid16 == GATT_UUID_HID_REPORT)
            return;
    }
#endif
...
{
    APPL_TRACE_DEBUG("bta_hh_le_open_cmpl");
...
BOOLEAN bta_hh_le_write_rpt_clt_cfg(tBTA_HH_DEV_CB *p_cb, UINT8 srvc_inst_id)
{
...
#ifdef BLUETOOTH_RTK
            /* 
               Do not enable both in Boot or Report Protocol Mode.
               Enable based on the Protocol Mode "p_cb->mode".
            */
            if( ( (p_cb->mode == BTA_HH_PROTO_RPT_MODE) && ((p_rpt->uuid != GATT_UUID_HID_BT_KB_INPUT) && (p_rpt->uuid != GATT_UUID_HID_BT_MOUSE_INPUT)) )
                    || ((p_cb->mode == BTA_HH_PROTO_BOOT_MODE) && (p_rpt->uuid != GATT_UUID_HID_REPORT))) 
            {
#endif
            if (p_rpt->uuid == GATT_UUID_BATTERY_LEVEL)
                srvc_uuid = UUID_SERVCLASS_BATTERY;
    ...
    if(cert_conf_mask&BLUETOOTH_RTK_BQB_PATCH_HOGP)
    {
        APPL_TRACE_ERROR("BQB:nothing to be writen in this service, try next service");
        if (++srvc_inst_id < BTA_HH_LE_HID_SRVC_MAX)
        {
            bta_hh_le_write_rpt_clt_cfg(p_cb, srvc_inst_id);
            return FALSE;
        }
        APPL_TRACE_ERROR("BQB:nothing to be writen");
    }
#endif
                                    ...
                                    void *p_ref_data, tBTM_STATUS result)
{
    ...
    APPL_TRACE_DEBUG("bta_hh_le_encrypt_cback result is %d", result);
...
void bta_hh_start_security(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_buf)
{
    ...
    UNUSED(p_buf);
    APPL_TRACE_DEBUG("%s p_dev_cb->disc_active = %d", __FUNCTION__,p_cb->disc_active);
    ...
    APPL_TRACE_DEBUG("%s p_dev_rec->sec_state = %d, sec_flag = %d", __FUNCTION__,p_dev_rec->sec_state, sec_flag);
...
/*******************************************************************************
**
** Function         bta_hh_le_direct_close
**
** Description      This function process the GATT close event and close directly,
**                  don't post it as a BTA HH internal event
**
** Parameters:
**
*******************************************************************************/
void bta_hh_le_direct_close(tBTA_GATTC_CLOSE * p_data)
{
    tBTA_HH_DEV_CB *p_dev_cb = bta_hh_le_find_dev_cb_by_bda(p_data->remote_bda);
    ...
    if (p_dev_cb != NULL)
    {
        APPL_TRACE_DEBUG("%s hid_handle = %d", __func__, p_dev_cb->hid_handle);
        memset(&p_buf, 0, sizeof(tBTA_HH_LE_CLOSE));
        p_buf.hdr.event            = sm_event;
        p_buf.hdr.layer_specific   = (UINT16)p_dev_cb->hid_handle;
        p_buf.conn_id              = p_data->conn_id;
        p_buf.reason               = p_data->reason;
        ...
        p_dev_cb->conn_id           = BTA_GATT_INVALID_CONN_ID;
        p_dev_cb->security_pending  = FALSE;
        bta_hh_sm_execute(p_dev_cb, sm_event, (tBTA_HH_DATA *)&p_buf);
    }
}
#endif
...
#ifdef BLUETOOTH_RTK_BQB
    if(char_uuid == GATT_UUID_PNP_ID){
        bta_hh_le_fill_16bits_srvc_id(TRUE, p_dev_cb->cur_srvc_index, UUID_SERVCLASS_DEVICE_INFO, &srvc_id);
        }
    else
#endif
    bta_hh_le_fill_16bits_srvc_id(TRUE, p_dev_cb->cur_srvc_index, UUID_SERVCLASS_LE_HID, &srvc_id);
    ...
    if (p_data->char_id.uuid.uu.uuid16 == GATT_UUID_BATTERY_LEVEL)
    {
        ...
        if(cert_conf_mask&BLUETOOTH_RTK_BQB_PATCH_HOGP)
        {
            if(bta_hh_le_read_char_dscrpt(p_dev_cb,
                    ...
                    GATT_UUID_BATTERY_LEVEL,
                    p_data->char_id.inst_id,
                    GATT_UUID_CHAR_CLIENT_CONFIG) != BTA_HH_OK)
            ...
            case GATT_UUID_HID_REPORT:
                if(cert_conf_mask&BLUETOOTH_RTK_BQB_PATCH_HOGP)
                {
                    if (bta_hh_le_read_char_dscrpt(p_dev_cb,
                            ...
                            p_data->char_id.inst_id,
                            GATT_UUID_CHAR_CLIENT_CONFIG) != BTA_HH_OK)
                        bta_hh_le_expl_rpt(p_dev_cb,
                            &(p_dev_cb->bqb_report_char_result),
                            &(p_dev_cb->bqb_report_char_cond),
                            p_dev_cb->bqb_report_prop);
                    return;
                }
                break;
#endif
...
void bta_hh_le_read_battery_level_descr_cmpl(tBTA_HH_DEV_CB *p_dev_cb, tBTA_GATTC_READ * p_data)
{
    ...
    if(cert_conf_mask&BLUETOOTH_RTK_BQB_PATCH_HOGP)
    {
        if (descr_uuid == GATT_UUID_CHAR_CLIENT_CONFIG)
            bta_hh_read_battery_level_cmpl(0, p_dev_cb, p_data);
        }
#endif
...
void bta_hh_w4_le_read_descr_cmpl(tBTA_HH_DEV_CB *p_dev_cb, tBTA_HH_DATA *p_buf)
{
...
#ifdef BLUETOOTH_RTK_BQB
    UINT8 input_report_data[5] = {0xAA, 0xB3, 0xF8, 0xA6, 0xCD};
    UINT8 output_report_data[7] = {0xEF, 0x90, 0x78, 0x56, 0x34, 0x12, 0x00};
    UINT8 feature_report_data[5] = {0xEA, 0x45, 0x3F, 0x2D, 0x87};
    UINT8 value[20];
#endif
    if (p_data == NULL)
        return;
    ...
    case GATT_UUID_HID_REPORT:
        ...
        if(cert_conf_mask&BLUETOOTH_RTK_BQB_PATCH_HOGP)
        {
            APPL_TRACE_ERROR("BQB:desc type is %x", p_data->descr_type.uuid.uu.uuid16);
            if(p_data->descr_type.uuid.uu.uuid16 == GATT_UUID_CHAR_CLIENT_CONFIG)
            {
                bta_hh_le_expl_rpt(p_dev_cb, &(p_dev_cb->bqb_report_char_result), &(p_dev_cb->bqb_report_char_cond), p_dev_cb->bqb_report_prop);
                break;
            }
            else
            {
                ...
                else if (p_rpt->rpt_type == BTA_HH_RPTT_OUTPUT)
                {
                    APPL_TRACE_ERROR("BQB:write command");
                    BTA_GATTC_WriteCharValue(p_dev_cb->conn_id,
                        &p_dev_cb->bqb_report_char_result,
                        BTA_GATTC_TYPE_WRITE_NO_RSP,
                        7,
                ...
                else if (p_rpt->rpt_type == BTA_HH_RPTT_FEATURE)
                {
                    BTA_GATTC_WriteCharValue(p_dev_cb->conn_id,
                        &p_dev_cb->bqb_report_char_result,
                        BTA_GATTC_TYPE_WRITE,
...
#endif
#ifdef BLUETOOTH_RTK
		//add by jim to test hidreports
		APPL_TRACE_DEBUG("jim@bta_hh_w4_le_read_descr_cmpl, char_id.inst_id=%02x", p_data->char_id.inst_id);
#endif
		...
		//add by jim to test hidreports
		{
			APPL_TRACE_DEBUG("jim@bta_hh_w4_le_read_descr_cmpl, found report entry & save it now");
#endif
            bta_hh_le_save_rpt_ref(p_dev_cb, p_rpt, p_data);
#ifdef BLUETOOTH_RTK
		//add by jim to test hidreports
...
void bta_hh_w4_le_write_cmpl(tBTA_HH_DEV_CB *p_dev_cb, tBTA_HH_DATA *p_buf)
{
    ...
    if(cert_conf_mask&BLUETOOTH_RTK_BQB_PATCH_HOGP)
    {
        if (p_data->char_id.uuid.uu.uuid16 == GATT_UUID_HID_REPORT)
        {
            APPL_TRACE_ERROR("BQB:write request");
            if (need_write == 1)
            {
                BTA_GATTC_WriteCharValue(p_dev_cb->conn_id,
                    &p_dev_cb->bqb_report_char_result,
...
void bta_hh_le_input_rpt_notify(tBTA_GATTC_NOTIFY *p_data)
{
    ...
    if(cert_conf_mask&BLUETOOTH_RTK_BQB_PATCH_HOGP)
    {
        for(hid_srvc_idx =0; hid_srvc_idx < p_dev_cb->total_srvc; hid_srvc_idx++)
        {
            p_rpt = bta_hh_le_find_report_entry(p_dev_cb,
                BTA_HH_LE_SRVC_DEF,
...
void bta_hh_le_open_fail(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_hh_le_open_fail p_cb->disc_active : %d", p_cb->disc_active);
...
{
    APPL_TRACE_DEBUG("bta_hh_le_api_disc_act p_cb->conn_id = %d", p_cb->conn_id);
...
static void bta_hh_le_add_dev_bg_conn(tBTA_HH_DEV_CB *p_cb, BOOLEAN check_bond)
{
    ...
    APPL_TRACE_DEBUG("bta_hh_le_add_dev_bg_conn check_bond: %d",check_bond);
    if (check_bond)
    {
...
{
    APPL_TRACE_DEBUG("bta_hh_le_remove_dev_bg_conn");
...
#ifdef BLUETOOTH_RTK
            bta_hh_le_direct_close(&p_data->close);
#else
                                    ...
                                    UINT8 proto_mode)
{
...
#if BTA_HH_DEBUG == TRUE
    APPL_TRACE_DEBUG("bta_hh_le_deregister_input_notif mode: %d", proto_mode);
#endif
    ...
    for (i = 0; i < BTA_HH_LE_RPT_MAX; i ++, p_rpt ++)
    {
        if (p_rpt->rpt_type == BTA_HH_RPTT_INPUT)
        {
            ...
            {
                /* deregister boot reports notification */
                if (p_rpt->uuid == GATT_UUID_HID_BT_KB_INPUT ||
                         p_rpt->uuid == GATT_UUID_HID_BT_MOUSE_INPUT)
                {
                    APPL_TRACE_DEBUG("<--- Deregister Boot Report ID: %d", p_rpt->rpt_id);
