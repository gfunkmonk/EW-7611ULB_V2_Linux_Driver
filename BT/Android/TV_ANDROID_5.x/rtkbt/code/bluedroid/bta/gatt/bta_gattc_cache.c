...
tBTA_GATT_STATUS bta_gattc_init_cache(tBTA_GATTC_SERV *p_srvc_cb)
{
    ...
    {
#ifdef BLUETOOTH_RTK
        // init p_srvc_list to all zero
        memset(p_srvc_cb->p_srvc_list, 0, BTA_GATTC_ATTR_LIST_SIZE);
#endif
...
void bta_gattc_disc_res_cback (UINT16 conn_id, tGATT_DISC_TYPE disc_type, tGATT_DISC_RES *p_data)
{
    ...
    APPL_TRACE_DEBUG("bta_gattc_disc_res_cback conn_id = %d, disc_type = %d",conn_id,disc_type);
...
void bta_gattc_disc_cmpl_cback (UINT16 conn_id, tGATT_DISC_TYPE disc_type, tGATT_STATUS status)
{
    ...
    APPL_TRACE_DEBUG("bta_gattc_disc_cmpl_cback conn_id = %d, disc_type = %d",conn_id,disc_type);
