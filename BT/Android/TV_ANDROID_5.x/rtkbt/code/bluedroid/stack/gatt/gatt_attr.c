...
static tGATT_PROFILE_CLCB *gatt_profile_find_clcb_by_conn_id(UINT16 conn_id)
{
    ...
    GATT_TRACE_EVENT("gatt_profile_find_clcb_by_conn_id : can't find p_clcb" );
#ifdef BLUETOOTH_RTK
    return NULL;
#else
    return p_clcb;
...
static tGATT_PROFILE_CLCB *gatt_profile_find_clcb_by_bd_addr(BD_ADDR bda, tBT_TRANSPORT transport)
{
    ...
    GATT_TRACE_EVENT("gatt_profile_find_clcb_by_bd_addr : can't find p_clcb" );
#ifdef BLUETOOTH_RTK
    return NULL;
#else
    return p_clcb;
                                           ...
                                           tGATT_STATUS status, tGATT_CL_COMPLETE *p_data)
{
    ...
    GATT_TRACE_DEBUG("%s() - p_clcb->conn_id : %d,op = %d", __FUNCTION__, p_clcb->conn_id, op);
#ifdef BLUETOOTH_RTK
