...
#define SMP_PAIR_PK_SIZE        (4 * BT_OCTET16_LEN + 1)
#define SMP_PAIR_DHK_SIZE       (BT_OCTET16_LEN + 1)
#define SMP_PAIR_KP_NOT_SIZE    (2)
#endif
...
#ifdef BLUETOOTH_RTK_SC
static BT_HDR * smp_build_pairing_public_key_cmd(UINT8 cmd_code, tSMP_CB *p_cb);
static BT_HDR * smp_build_pairing_DHkey_check_cmd(UINT8 cmd_code, tSMP_CB *p_cb);
static BT_HDR * smp_build_pairing_keypress_notif_cmd(UINT8 cmd_code, tSMP_CB *p_cb);
#endif
...
#ifdef BLUETOOTH_RTK_SC
    ,smp_build_pairing_public_key_cmd,  /* 0x0C: pairing Public Key*/
    smp_build_pairing_DHkey_check_cmd,  /*0x0D: pairing DHKey check*/
    smp_build_pairing_keypress_notif_cmd    /*0x0E pairing keypress notification*/
#endif
};
/*******************************************************************************
**
...
/*******************************************************************************
**
** Function         smp_build_pairing_public_key_cmd
**
...
static BT_HDR * smp_build_pairing_public_key_cmd(UINT8 cmd_code, tSMP_CB *p_cb)
{
    ...
    SMP_TRACE_EVENT("smp_build_pairing_public_key_cmd");

    if ((p_buf = (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + SMP_PAIR_PK_SIZE + L2CAP_MIN_OFFSET)) != NULL)
    {
        p = (UINT8 *)(p_buf + 1) + L2CAP_MIN_OFFSET;

        UINT8_TO_STREAM (p, SMP_OPCODE_PAIRING_PUB_KEY);
        ARRAY_TO_STREAM (p, p_cb->local_pk, BT_OCTET64_LEN);

        p_buf->offset = L2CAP_MIN_OFFSET;
        p_buf->len = SMP_PAIR_PK_SIZE;

        SMP_TRACE_EVENT("opcode=%d auth_req=0x%x",SMP_OPCODE_PAIRING_PUB_KEY,  p_cb->loc_auth_req );
    }
...
static BT_HDR * smp_build_pairing_DHkey_check_cmd(UINT8 cmd_code, tSMP_CB *p_cb)
{
    ...
    SMP_TRACE_EVENT("smp_build_pairing_DHkey_check_cmd");

    if ((p_buf = (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + SMP_PAIR_DHK_SIZE + L2CAP_MIN_OFFSET)) != NULL)
    {
        p = (UINT8 *)(p_buf + 1) + L2CAP_MIN_OFFSET;

        UINT8_TO_STREAM (p, SMP_OPCODE_PAIRING_DHK_CHECK);
        ARRAY_TO_STREAM (p, p_cb->dhk_check, BT_OCTET16_LEN);

        p_buf->offset = L2CAP_MIN_OFFSET;
        p_buf->len = SMP_PAIR_DHK_SIZE;

        SMP_TRACE_EVENT("opcode=%d auth_req=0x%x",SMP_OPCODE_PAIRING_DHK_CHECK,  p_cb->loc_auth_req );
    }
