...
#ifdef BLUETOOTH_RTK_SC
    if(((smp_cb.role == HCI_ROLE_MASTER && smp_cb.state == SMP_ST_RAND) ||
        (smp_cb.role == HCI_ROLE_SLAVE && smp_cb.state == SMP_ST_ENC_PENDING)) &&
        smp_cb.cb_evt == SMP_PASSKEY_CFM_EVENT &&
        !memcmp (smp_cb.pairing_bda, bd_addr, BD_ADDR_LEN)) {
        /* clear the SMP_PASSKEY_CFM_EVENT event after get grant */
