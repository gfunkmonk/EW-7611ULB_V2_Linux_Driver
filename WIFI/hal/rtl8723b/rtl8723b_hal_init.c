...
static int _BlockWrite(struct _ADAPTER *padapter, void *buffer, u32 buffSize)
{
	...
	if (remainSize_p1) {
		...
		for (i = 0; i < blockCount_p2; i++) {
			ret = rtw_writeN(padapter,
					 (FW_8723B_START_ADDRESS + offset + i * blockSize_p2),
					 blockSize_p2,
...
)
{
	...
	/* Init Tx boundary. */
	PlatformEFIOWrite1Byte(Adapter, REG_DWBCN0_CTRL_8723B + 1, (u8)txpktbuf_bndy);
#endif
...
s32 rtl8723b_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	if ((pFirmware->ulFwLength - 32) > FW_8723B_SIZE) {
		rtStatus = _FAIL;
	...
	if (pxmitframe->frame_tag == DATA_FRAMETAG) {
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if((pattrib->ether_type == ETH_P_ARP) &&
				...
				SET_TX_DESC_TX_RATE_8723B(pbuf, MRateToHwRate(IEEE80211_OFDM_RATE_6MB));
			 else
...
void rtl8723b_read_wmmedca_reg(PADAPTER adapter, u16 *vo_params, u16 *vi_params, u16 *be_params, u16 *bk_params)
{
	u8 vo_reg_params[4];
	u8 vi_reg_params[4];
	u8 be_reg_params[4];
	...
	GetHwReg8723B(adapter, HW_VAR_AC_PARAM_VO, vo_reg_params);
	GetHwReg8723B(adapter, HW_VAR_AC_PARAM_VI, vi_reg_params);
	GetHwReg8723B(adapter, HW_VAR_AC_PARAM_BE, be_reg_params);
	GetHwReg8723B(adapter, HW_VAR_AC_PARAM_BK, bk_reg_params);
...
void rtl8723b_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->read_wmmedca_reg = &rtl8723b_read_wmmedca_reg;
