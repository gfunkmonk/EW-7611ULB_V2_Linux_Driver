...
static int rtw_net_set_mac_address(struct net_device *pnetdev, void *addr)
{
	...
	rtw_set_mac_addr(pnetdev, sa->sa_data); /* set mac addr to net_device */
...
int rtw_os_ndev_register(_adapter *adapter, const char *name)
{
	...
	rtw_set_mac_addr(ndev, adapter_mac_addr(adapter));
...
int _netdev_vir_if_open(struct net_device *pnetdev)
{
	...
	if (padapter->bup == _FALSE) {
		...
		*/
		...
		rtw_set_mac_addr(pnetdev, adapter_mac_addr(padapter));
	}
#endif /*CONFIG_PLATFORM_INTEL_BYT*/
