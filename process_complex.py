...
def extract_config_names(line):
    """Extract all CONFIG_RTL* names from a line."""
    return re.findall(r'CONFIG_RTL\w+', line)

def all_configs_unused(configs):
    """Check if all configs in list are unused."""
...
def has_8723d(configs):
    """Check if 8723D is in the config list."""
    ...
    while i < len(lines):
        ...
        if stripped.startswith(('#if ', '#ifdef ', '#elif ')):
            ...
            if configs and all_configs_unused(configs) and not has_8723d(configs):
                # All configs are unused, skip this block
                start_i = i
                depth = 1
                i += 1
                ...
                while i < len(lines) and depth > 0:
                    ...
                    if inner.startswith(('#ifdef ', '#if ')):
                        ...
                        if elif_configs and not all_configs_unused(elif_configs):
                            # This elif is good, convert to #if
...
# Process remaining complex files
files = [
    'WIFI/hal/hal_com.c',
    'WIFI/core/rtw_xmit.c',
    'WIFI/hal/hal_btcoex_wifionly.c',
    'WIFI/hal/hal_dm.c',
    'WIFI/platform/platform_ARM_SUNxI_usb.c',
    'WIFI/platform/platform_sprd_sdio.c',
    'WIFI/platform/platform_ARM_SUNxI_sdio.c',
    'WIFI/platform/platform_ARM_SUNnI_sdio.c',
]
